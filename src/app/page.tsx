...
function OddsChip({ game }: { game: Game }) {
  ...
  return (
    <div className="text-right">
      <div className="text-sm font-bold text-[var(--osu-scarlet)]">{details || "Line TBD"}</div>
      {overUnder != null && (
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <div className="font-semibold text-white truncate">
            {atOrVs} {rank && <span className="text-[var(--osu-scarlet)]">No. {rank} </span>}
            {game.opponent.name}
          </div>
...
function RosterColumn({ title, players }: { title: string; players: Player[] }) {
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--osu-scarlet)] mb-3">
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rankings.map((r) => {
            const isOSU = r.teamId === "194";
            return (
              <tr
                key={r.teamId}
                className={`border-b border-slate-800/50 ${isOSU ? "bg-[#BB0000]/15" : ""}`}
              >
                ...
                <td className="py-2 font-medium">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={isOSU ? "text-white font-bold" : "text-slate-300"}>
                      {r.team}
                    </span>
                  </div>
                </td>
  ...
  return (
    <main className="min-h-screen">
      <header
        style={{
          background:
            "linear-gradient(135deg, #BB0000 0%, #3d0000 55%, #101010 100%)",
        }}
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-lg sm:text-3xl font-black tracking-tight flex items-center gap-1.5 sm:gap-2 flex-wrap">
                {osuRank && (
                  <span className="text-sm sm:text-lg font-black text-[var(--osu-scarlet)]">No. {osuRank}</span>
                )}
                Ohio State Buckeyes
              </h1>
            ...
            {record && (
              <div className="text-right flex-shrink-0">
                <div className="text-xl sm:text-4xl font-black tabular-nums text-[var(--osu-scarlet)]">
                  {record}
                </div>
