...
const TEAM_ID = "194"; // Ohio State Buckeyes
...
export async function getTeamInfo(): Promise<TeamInfo> {
  ...
  return {
    name: String(team.displayName ?? "Ohio State Buckeyes"),
    abbreviation: String(team.abbreviation ?? "OSU"),
    logo: String(logos[0]?.href ?? ""),
    color: String(team.color ?? "BA0C2F"),
    altColor: String(team.alternateColor ?? "666666"),
    record: "",
  };
}
